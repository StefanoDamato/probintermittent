...
#' @family models
#' @export
seasonal = function(data, h=10, levels=0.9, holdout=FALSE,
                         cumulative=FALSE, side=c("upper", "both", "lower"),
                         nsim=10000, paths=TRUE, freq=c('D', 'W', 'M')){
    ...
    # Select the appropriate period length
    period <- c('D'=7, 'W'=52.14, 'M'=12)[freq];
    params = list('period' = period);
    forecast_samples = matrix(NA, nsim, h);

    # Generate forecast sampling from seasonal data
    for (i in 1:min(period, h)){
      times_f <- seq(i, h, period);
      y_season <- y_insample[seq(L-period+i, 1, -period)];
      forecast_samples[,times_f] <- sample(y_season, nsim*length(times_f),
                                           replace=TRUE);
    }
    ...
    model <- list("model" = "Seasonal", "call" = call, "params" = params,
